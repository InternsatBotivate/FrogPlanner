...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (new Date(row.expires_at) < new Date()) {
      return page(res, 400, 'error', 'This link has expired. Request a new one from Frog Planner settings.');
    }
    ...
    return page(res, 200, 'ok', 'Your email is verified — Frog Planner reminders are now enabled.');
  } catch (e) {
